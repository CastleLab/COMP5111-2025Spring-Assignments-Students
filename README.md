# HKUST COMP5111 (2025Spring) Assignment 2

## Deadline: 11:55 pm 10 May 2025

## Assignment Objective

In assignment 2, you are expected to complete three compulsory tasks: 
1. (Task 1) 
...
2. (Task 2)
   ...
   Your tool should be able to help you locate our injected faults, and you will eventually fix these faults. 
   Besides, you will compare the performance of a reasoning LLM and a non-reasoning LLM in this task and write a report on your usage and findings.
3. (Task 3)
    You will extend the test suite to improve the ranking of fault localization, understanding the limitations of automatically generated tests and the noises in fault localization. Please take advantage of appropriate reasoning or non-reasoning LLMs to finish the goal. Write a report on your comparison and usage of non-reasoning and reasoning LLMs.
...
2. If your problem is not solved, you are recommended to create `Issues` in this repository.
   Issues shall be attended to by the TA within 24 hours. You are highly encouraged to start early to accommodate unexpected uncertainties arising from hardware and software.
...
5. Some potentially helpful links to access LLMs: https://genai.ust.hk/, https://poe.com/, https://www.deepseek.com/, https://openrouter.ai/, https://lmarena.ai/.
...
#### Requirements
- Use Evosuite to construct **three test suites** for CUT.
...
#### Submissions: 
The three test suites generated by you, i.e., the folder `generated-evosuite0`, `generated-evosuite1`, and `generated-evosuite2` in `src/test`.
...
### Task 2: Fault Localization based on Soot (55%).
...
#### Requirements
- Instrument CUT using soot to collect necessary information (e.g., how many times a statement was executed by the passed and failed test cases) during the running of tests, and run the **six fault-revealing test suites** we provide to collect the information.
...
- Localize the faults injected by us based on the spectrum and fix them. Write a brief report to list the found faults and your fixing patches.
- Compare the helpfulness of **one** non-reasoning LLM (e.g., DeepSeek-V3, Qwen2.5-Coder-32B, and gpt-4o-mini) and **one** reasoning LLM (e.g., DeepSeek-R1, QwQ-32B, and Grok-3) for fault localization. Write a report to share your findings on the strengths and limitations of the two LLMs on the task. Also, give a final recommendation on the LLM to use and justify your recommendation in the report.
...
The report should provide useful information indicating where faults reside in the source code. There are **multiple faults** injected in the subject program and each of them is **in a single line**. It is possible that some test suites *CANNOT* reveal *ALL* injected faults, but we ensure that each fault that you need to locate and fix must be revealed by *AT LEAST ONE* test suite. In other words, **only the buggy code statements that trigger the failure on our provided tests should be located and fixed**; the remaining code is considered correct and in no need of fault localization and fixing.
We do not tell you the exact number of injected faults, and you need to locate and fix as many faults as you can. But the false report of the statements that *do not cause any test failure on the six test suites will be penalized*.
...
#### Submissions
1. The source code to collect useful information and generate the spectrum reports. You can build your program upon your code in assignment 1. Scripts to run your program and readme are required.
2. The spectrum reports of potential faulty statements running against **each** of the six test suits provided by us. In total, you need to submit **six spectrum reports**. Please name each of them in the format `spectrum_fl_ochiai_randoop[0-2].tsv` and `spectrum_fl_ochiai_evosuite[0-2].tsv`. 
    - Each spectrum report should be in `tsv` format and each line should be in the format of "method signature[TAB]statement[TAB]suspicious score[TAB]ranking". 
    - The report should be sorted according to the descending order of suspicious scores. If multiple statements have the same scores, please sort them according to the alphabetical order of method signature and statement.
...
3. The fault localization reports of the faults found and fixed by you. For each fault, you should create a `.txt` file and name it in the format `fault_[line-number].txt`, where `line-number` is the line number of the fault you found. In each text file, please put down:
...
4. An LLM comparison and usage report for this task. You should create only one `.pdf` file and name it as `LLMForTask2.pdf`. Please report:
    ...
    - The non-reasoning LLM and reasoning LLM you compare.
    - The prompt that you use to solve the problem;
    - The comparison process and conclusion (i.e., the final LLM that you suggest to leverage). You may compare LLMs based on their fault localization results (e.g., the ZZ ranking of faults generated xx LLM is better than the original; the ABC info generated by xx LLM helps me understand PP and guide my QQ; meanwhile, the yy LLM cannot generate clear AA information, e.g., .... yy LLM can generate BB information, but the information is not as clear as ... by xx LLM ...). You may find evidence on why the reasoning LLM succeeds or fails based on its reasoning procedure. There can be 200-400 words (not a strict limitation, try to be informative but concise). 
    - **You are prohibited from using LLMs to synthesize any content other than their outputs in the report.**
...
3. *Quality of your LLM comparison and usage report* (15%): 5% will be earned if you have tried both reasoning and non-reasoning LLMs in the fault localization procedure. The other 10% will be earned if your comparison clearly demonstrates the strengths and weaknesses of two types of LLMs in fault localization and you make a reasonable final decision.
...
- Similar to Task 2, compare the helpfulness of **one** non-reasoning LLM (e.g., DeepSeek-V3, Qwen2.5-Coder-32B, and gpt-4o-mini) and **one** reasoning LLM (e.g., DeepSeek-R1, QwQ-32B, and Grok-3) for test suite extension. Write a report to share your findings on the strengths and limitations of the two LLMs on the task. Also, give a final recommendation on the LLM to use and justify your recommendation in the report.
...
4. An LLM comparison and usage report for this task. You should create only one `.pdf` file and name it as `LLMForTask3.pdf`. Please report:
    ...
    - The non-reasoning LLM and reasoning LLM you compare.
    - The prompt that you use to solve the problem;
    - The comparison process and conclusion (i.e., the final LLM that you suggest to leverage). You may compare how two LLMs boost the fault localization procedure (e.g., the ABC info generated by xx LLM helps me understand XYZ and get a possible value for PP, and finally guide my QQ; even if yy LLM can also ..., it suffers from limitations like ...). You may find evidence on why the reasoning LLM succeeds or fails based on its reasoning procedure. There can be 200-400 words (not a strict limitation, try to be informative but concise). 
    - **You are prohibited from using LLMs to synthesize any content other than their outputs in the report.**
...
* *Quality of your LLM comparison and usage report* (10%): 5% will be earned if you have tried both reasoning and non-reasoning LLMs in the test extension procedure. The other 5% will be earned if your comparison clearly demonstrates the pros and cons of two types of LLMs in test extension and you make a reasonable final decision.
...
3. A `README-student.md` explaining how to run your code. Put your running scripts (if needed) under `${PROJECT_ROOT}/scripts`
4. Put your screenshot into `${PROJECT_ROOT}/screenshots`
5. Put the test suites generated by you into `${PROJECT_ROOT}/src/test/generated-evosuite[0-2]`, and the test suite refined by you into `${PROJECT_ROOT}/src/test/refined-[randoop|evosuite][0-2]`.
...
9. Put the LLM usage reports (for Tasks 2 and 3) in `${PROJECT_ROOT}/reports/LLM`.
